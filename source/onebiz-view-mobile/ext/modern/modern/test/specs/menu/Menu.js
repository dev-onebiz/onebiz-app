...
topSuite("Ext.menu.Menu", ['Ext.Button', 'Ext.field.Text', 'Ext.scroll.Scroller'], function() {
    ...
    describe('simple show of floated menu', function() {
        ...
        it('should show and be constrained to the body and within float insets', function() {
            var docInsets = Ext.Widget.floatInset,
                expectedRegion = Ext.getBody().getRegion();
        ...
        describe('as a child of another component', function() {
            var byCmp;
            ...
            it('should work as a child of another component', function() {
                menu.showBy(byCmp);

                expect(menu.getFloated()).toBe(true);
    ...
    describe("defaultType", function() {
        ...
        xit("should create a check item if there is a checked property and no xtype", function() {
            makeMenu({
                items: [{
                    checked: true
                }, {
    ...
    describe('Touch events', function() {
        ...
        if (jasmine.supportsTouch) {
            xit("should not expand submenu when parent item is touched", function() {
                makeMenu({
                    items: [{
                        xtype: 'datefield'
                    }]
                ...
                var field = menu.down('datefield'), // TODO: Reinstate when there's a DateField to speak of!
    ...
    xdescribe("moving menu", function() {
    ...
    describe('hiding all other menus', function() {
        ...
        xit("should not hide menus when they are nested as part of other components", function() {
            makeMenu({
                items: {
                    xtype: 'container',
                    items: [{
    ...
    describe("hiding when doing an action outside the menu", function() {
        it("should hide the menu", function() {
            ...
            menu.show();
            doElementMousedown(field.inputElement);
            expect(menu.isVisible()).toBe(false);
            doElementMouseup(field.inputElement);
            field.destroy();
        });
    ...
    (Ext.isIE ? xdescribe : describe)('navigation', function () {
        ...
        it("should navigate when a child item has an href config", function() {
            ...
            jasmine.fireMouseEvent(menu.items.getAt(0).bodyElement.dom, 'click');
    ...
    xdescribe('Multi level menus', function() {
        var testContainer,
            button,
            secondButton,
            topMenu,
    ...
    describe("Disabling tabbing", function() {
        ...
        it('should set tabIndex to -1 on disabled items', function () {
            ...
            runs(function () {
                ...
                expect(submenu.focusEl.dom.tabIndex).toBe(-1);
            });
        });
    });

    xdescribe("shortcut keys", function() {
    ...
    describe('rehoming a submenu', function() {
        ...
        it("should move the submenu into the new parent menu's floatWrap", function() {
            makeMenu({
                items: [{
                    itemId: 'item1',
                    text: 'Item 1',
            ...
            var item1 = menu.getComponent('item1');

            menu.show();

            item1.expandMenu();

            // Must be last child node of owning Menu's floatWrap
            expect(item1.getMenu().floatWrap.dom.parentNode === menu.floatWrap.dom).toBe(true);
            expect(item1.getMenu().floatWrap.dom.nextSibling).toBe(null);
            item1.getMenu().hide();

            otherMenu = new Ext.menu.Menu({});
            otherMenu.add(item1);
            otherMenu.show();

            item1.expandMenu();

            // The descendant floated Menu must be inside the floatWrap of its closest
            // floated ancestor which is now "otherMenu".
            // Must now be last child node of OtherMenu's floatWrap
            expect(item1.getMenu().floatWrap.dom.parentNode === otherMenu.floatWrap.dom).toBe(true);
            expect(item1.getMenu().floatWrap.dom.nextSibling).toBe(null);
        });
    });
